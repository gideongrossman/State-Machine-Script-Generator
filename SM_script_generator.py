...
// http://www.velocitek.com/style-guide/c_style_guide.xml\n\n'
        ...
        for state in capitalized_states:
            ...
            enum_type_definition + '%s_STATE,\n' %(state)
        enum_type_definition = enum_type_definition + '}\n'
        
        capitalized_filename_no_underscores =(
                        CapitalizeFirstLettersRemoveUnderscores(self.filename))
        ...
        f.write('uint8_t Run' + capitalized_filename_no_underscores + 
            'SM(uint8_t current_event);\n')
        f.write('void Start' + 
            capitalized_filename_no_underscores + 'SM(void);\n')
        f.write(capitalized_filename_no_underscores + 'States Query'
            + capitalized_filename_no_underscores + 'SM(void);\n\n')
        f.write('#endif')
        f.close()
    ...
    def CreateImplementationFile(self):
        ...
        boilerplate_comments = ('// Copyright (c) ' + self.year + 
            ' Velocitek Inc. All rights reserved.\n\
...
// http://www.velocitek.com/style-guide/c_style_guide.xml\n\n')
        ...
        for state in self.states:
            g.write('static void During' + 
                CapitalizeFirstLettersRemoveUnderscores(state) + 
                '(uint8_t event);\n')
            
        g.write('\n// Module Variables' + '-'*60 + '\n')
        g.write('static ' + 
            CapitalizeFirstLettersRemoveUnderscores(self.filename) + 
            'States current_' + self.filename + '_state_ = ' + 
            self.states[0].upper() + '_STATE;' + '\n\n')
        ...
        g.write('{\n  ' + 
            CapitalizeFirstLettersRemoveUnderscores(self.filename) + 
            'States next_state = current_' + self.filename + 
            '_state_;\n\n')
      ...
      {\n        switch (current_event)\n        {\n')
        ...
        g.write('  }\n\n')
        g.write('  if (next_state != current_' + 
            self.filename + '_state_)\n  {\n')
        g.write('    switch (current_' + self.filename + 
            '_state_)\n    {\n')
        for state in self.states:
            g.write('      case ' + state.upper() + 
                '_STATE' + ':\n        During' + 
                CapitalizeFirstLettersRemoveUnderscores(state) + 
                '(EV_EXIT);\n        break;\n\n')
        g.write('      default:\n        break;\n    }\n\n')
        ...
        g.write('void Start' + 
            CapitalizeFirstLettersRemoveUnderscores(self.filename) + 
            'SM(void)\n{\n')
        g.write('  current_' + self.filename + '_state_ = ' + 
            self.states[0].upper() + '_STATE' + ';\n')
        g.write('  Run' + 
            CapitalizeFirstLettersRemoveUnderscores(self.filename) + 
            'SM(EV_ENTRY);\n}\n\n')
        g.write(CapitalizeFirstLettersRemoveUnderscores(self.filename) + 
            'States Query' + 
            CapitalizeFirstLettersRemoveUnderscores(self.filename) + 
            'SM(void)\n')
